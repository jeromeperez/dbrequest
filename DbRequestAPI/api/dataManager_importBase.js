...
var funcUtil3 = require('../utilitaires/verifImportation');
...
module.exports.route = {
	...
	handler: function (request, reply) {
		...
		var RequeteSurDataManager = function() {
			...
			HttpRequest(options, function (error, response, body) {
				if (!error && body.ok) {
					...
					var d = new Date();
					var extentionDate = '_' + d.getFullYear() + '_' + (parseInt(d.getMonth()) + 01).formatNombre() + '_' 						+ d.getDate().formatNombre() + '_' + d.getHours().formatNombre() + 'h_' + d.getMinutes().formatNombre() + 'm_'
					+ d.getSeconds().formatNombre() + 's';

					funcUtil2.suiviImportation('creationJsonBase', nomBaseDataManager, mmids, monNouvelleBase, extentionDate);


					HttpRequest(urlRequeteAlldocs, function (error2, response2, body2) {
						if (!error2 && response2.statusCode == 200) {
							...
							var d1 = new Date();
    							console.log(d1) ;
							ImportVersDbRequest(json , extentionDate);
							...
							console.error('Echec de la requête', error2);
							return reply(null);
						};
					});
				} else {
		...
		var ImportVersDbRequest = function(jsonBase , extentionDate) {
					...
					, function(err, result) {
						...
						client.query('CREATE TABLE ' + nomBaseImport + '(numligne integer NOT NULL, lignejsonb text, ' +
							...
							, function(err2, result2) {
								if(err2) {
									return console.error('erreur creation table ', err2);
								};
								...
								async.forEachOf(jsonBase.rows, function (ligne,i,callback) {
									...
									, function(err3, result3) {
										if(err3) {
											return console.error('erreur query ', err3);
										};
										callback();
									})
								}, function (err4) {
									if (err4) console.error(err.message);
		...
		var siVerifOK = function () {
			console.log('Debut verifOk');
  			return funcUtil3.verifImportation().then(function (){
				console.log('Fin verifOk');
